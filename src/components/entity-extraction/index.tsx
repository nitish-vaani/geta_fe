// // frontend/src/components/entity-extraction/index.tsx
...
//       {entityEntries.some(([, data]) => !data.value || data.value === "Not Mentioned") && (
...
const EntityExtraction: React.FC<EntityExtractionProps> = ({ entities }) => {
  ...
  // Check if we have lead-related or confidence data to show info box
  const hasLeadData = entityEntries.some(([key]) => 
  ...
  return (
    <div className="entity-extraction">
      ...
      {/* Info box for confidence scoring - only show when relevant */}
      {hasLeadData && (
        <div className="confidence-info-box">
          <strong>Note:</strong> Confidence levels: Scores ≥0.9 = HIGH, 0.75-0.9 = MEDIUM, &lt;0.75 = LOW.
        </div>
      )}
        ...
        {entityEntries.map(([fieldName, data]) => {
          ...
          return (
            <div key={fieldName} className="entity-card">
              ...
              {data.text && data.text !== "NA" && data.text !== "Not Mentioned" && data.text.trim() !== "" && (
                <div className="entity-source">
                  <span className="source-label">From transcript:</span>
                  <span className="source-text">"{data.text}"</span>
                </div>
